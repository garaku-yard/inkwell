...
- **Google Drive backup (Settings → Integrations "#16") — SCOPED, not built.** Full design in [`.orbit/docs/log/2026-06-22-drive-backup-design.md`](.orbit/docs/log/2026-06-22-drive-backup-design.md). One-way backup of all projects to the user's *own* Drive (vault → real files; non-vault → lossless `.inkwell.json` **+** readable `.pdf`), manual trigger, desktop-only, `drive.file` scope ⇒ **$0** (no paid CASA audit — that's only for restricted full-`drive` scope). Needs a free Google Cloud OAuth client (operator step); build inert-until-set, like Paddle. ~2 sessions when picked up.
