...
| Prose, Poetry / Lyrics, Comic, TTRPG, Interactive Fiction, Memoir | ✅ | per-format element vocabularies, screenplay-class keyboard parity, format-specific export (EPUB / ChordPro / CBZ / Twee / txt / md). **Design pass underway** — Prose (Novel/Memoir) now on A4 paginated pages + a page-edge tool rail (`4d80a67`); rolling out to the rest |
...
- **Editor UI/UX consistency + brand identity — IN PROGRESS.** The editor surfaces felt off and diverged a lot across the nine formats: each editor (Prose / Poetry / Comic / TTRPG / IF / Memoir / Lyrics / Vault) looked and behaved differently from the screenplay editor, which reads as the "reference." Goal: a single, intentional editor design language — consistent canvas, typography rhythm, element/toolbar treatment, spacing, empty states, and interaction model — shared by all nine, so they feel like one product. **Prototyped on Prose first (`4d80a67`):** two shared primitives now exist — (1) the writing surface is **discrete A4 paginated pages** (`210×297mm`, min-height so estimated breaks never clip; each chapter opens a fresh sheet), mirroring the screenplay's paginated `EditorPane`; (2) **`EditorToolRail`** — an element toolbar that rides the page's right edge in the margin (pinned card / collapse-to-handle / hover flyout; per-kind persisted), replacing the inline ghost-button rows. **Remaining:** roll the page + rail out to Poetry/Lyrics, Comic, TTRPG (swap its `SlashMenu` for the rail), and IF (Memoir rides Prose; Screenplay is the reference and Vault is markdown/CM6 — both left as-is); then define the canvas in `BRANDBOOK.md` and pin the visual contract in the smoke harness.
