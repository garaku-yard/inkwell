...
- **Editor UI/UX consistency + brand identity — IN PROGRESS.** One intentional editor design language shared across the nine formats, so they feel like one product (Screenplay is the reference; Vault is CM6/markdown — both left as-is). **Shipped so far:**
  ...
  - **A4 page + tool rail — shared primitives extracted** (`lib/editor/paginate.ts` + `components/editor/shared/PagedSheets.tsx`): discrete A4 sheets (`210×297mm`, min-height so estimated breaks never clip) on a desk with the rail in the margin. **Prose** and **Poetry** now render on them.
  - **RESUME POINT: convert Comic, TTRPG, IF to A4 + rail via `PagedSheets`** — each gets its own block union (with a `key` field), pagination estimate, `renderBlock`, format `RailItem[]`, and `handleRailSelect`; TTRPG swaps its `SlashMenu` for the rail; IF puts its *active passage* on a single sheet (write view only). Then define the canvas in `BRANDBOOK.md` and pin the visual contract in the smoke harness.
