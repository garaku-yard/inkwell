...
| Cloud sync (opt-in, desktop) | ⚠️ | two engines built + server-verified: DB-row sync (incremental `sync_outbox` push, tombstones, last-sync-wins) + path-keyed **vault file sync** (md + binary attachments). **Two-device app-level verify on a running desktop build still pending.** See `SYNC_DESIGN.md` + Open/deferred |
...
- **Google Drive backup (Settings → Integrations "#16") — SCOPED, not built.** Full design in `DRIVE_BACKUP.md`. One-way backup of all projects to the user's *own* Drive (vault → real files; non-vault → lossless `.inkwell.json` **+** readable `.pdf`), manual trigger, desktop-only, `drive.file` scope ⇒ **$0** (no paid CASA audit — that's only for restricted full-`drive` scope). Needs a free Google Cloud OAuth client (operator step); build inert-until-set, like Paddle. ~2 sessions when picked up.
