...
- **Open-core monetization** — tier count, pricing, free/paid split, gate points (needs product input). **Quota enforcement is blocked on this:** only `MetricProjects` has a call site (a no-op until tiers define limits); collaborators / ai_tokens / exports each carry a structural blocker (per-project vs user-level metric mismatch; no Plan proto field + BYO-key product question; needs monthly-windowed usage tracking).
