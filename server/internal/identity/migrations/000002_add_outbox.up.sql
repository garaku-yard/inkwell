-- identity_outbox stores domain events that must be reliably published to Kafka.
-- Events are written in the same DB transaction as the triggering business write
-- (Register), then a background poller reads unpublished rows, emits them to
-- Kafka, and marks them published. This prevents event loss if the service
-- crashes between the two steps.
--
